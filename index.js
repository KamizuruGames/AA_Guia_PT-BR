...
const ThirdBossActions = {						// Third Boss Attack Actions
	113: {msg: 'FRENTE, STUN ATRAS', msg_ru: 'Передний, Задний'},
	111: {msg: 'DIREITA SEGURO , FORA SEGURO', msg_ru: 'Право СЕЙФ , Наружу СЕЙФ', sign_degrees: 90, sign_distance: 190},
	109: {msg: 'ESQUERDA SEGURO , DENTRO SEGURO', msg_ru: 'Лево СЕЙФ , Внутрь СЕЙФ', sign_degrees: 270, sign_distance: 110}
};

const ThirdBossTwoUp = {
	104: {msg: 'STUN ATRAS', msg_ru: 'Задний'}
};
...
module.exports = function antaroth_guide(dispatch) {
	...
	dispatch.hook('S_LOAD_TOPO', 3, (event) => {
		...
		{								
			insidemap = true;
			command.message('Bem vindo a Antaroth - Normal Mode');
			load();
		} 
		else if (event.zone === mapID[1]) {
			insidemap = true;
			command.message('Bem vindo a Antaroth - Hard Mode');
			load();
		}
		else if (event.zone === mapID[2]) {
			insidemap = true;
			command.message('Bem vindo a Antaroth - 7 man');
			load();
		} 
		else
		{
	...
	command.add('aaguide', () => {
		if(!insidemap) { command.message('Voce dever estar dentro da Antaroth'); return; }
		enabled = !enabled;
		command.message('Guia Antaroth '+(enabled ? 'Ativado' : 'Desativado') + '.');
	});
	
	command.add('itemhelp', () => {
		if(!insidemap) { command.message('Voce dever estar dentro da Antaroth'); return; }
		itemhelper = !itemhelper;
		command.message('SINAIS DO LOCAL SEGURO '+(itemhelper ? 'Ativado' : 'Desativado') + '.');
	});
	...
	command.add('toparty', (arg) => {
		...
		if(arg === "stream")
		{
			...
			sendToParty = false;
			itemhelper = false;
			command.message((streamenabled ? 'Stream mode Ativado' : 'Stream mode Desligado'));
		}
		else
		{
			streamenabled = false;
			sendToParty = !sendToParty;
			command.message((sendToParty ? 'Guia Antaroth - As mensagens serao enviadas para a party' : 'Guia Antaroth - Somente você vera mensagens'));
		}
	});
